...
window = ObjGui.Window('CCTV', layout,location = (0,0) , element_justification='center', finalize=True, resizable=True)
window['-VID_OUT-'].expand(True, True)                


camera_1 = cv2.VideoCapture(CAM_URL_1)
time.sleep(1)
camera_2 = cv2.VideoCapture(CAM_URL_2)
time.sleep(1)
...
while True:                    # The PSG "Event Loop"
    ...
    if(ret2):
        ...
        window.FindElement('-VID_OUT1-').Update(data=cv2.imencode('.png',cv2.resize(frame2, OPDimenssion, interpolation = ResizeInterpolation))[1].tobytes()) # Update image in window
window.close()
